...
bot.command('menu', (ctx) => {
    if (ctx.message.chat.id.toString() === chatId) {
        ctx.reply('Добро пожаловать!', {
            reply_markup: {
                inline_keyboard: [
                    [{ text: "🆕 Новый", callback_data: 'new' }, { text: "📝 Записать", callback_data: 'record' }], 
                    [{ text: "ℹ️ Информация", callback_data: 'info' }, { text: "🔍 Поиск", callback_data: 'search' }], 
                    [{ text: "❓ Справка", callback_data: 'help' }, { text: "⚙️ Настройки", callback_data: 'settings' }] 
                ]
            }
        });
    } else {
    ...
    if (ctx.callbackQuery.message.chat.id.toString() === chatId) {
        ...
        bot.on('text', (ctx) => {
            if (!userData[ctx.from.id].name) {
                ...
                //  URL для запроса
                const addClientURL = `${config.serverURL}/add?name=${userData[ctx.from.id].name}&group=${userData[ctx.from.id].group}`;

                // POST запрос
                axios.post(addClientURL)
                    .then(response => {
                        ctx.reply(`Код: ${response.data.code}`);
                    })
                    .catch(error => {
                        console.log(error);
